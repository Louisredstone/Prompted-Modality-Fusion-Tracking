...
from ..utils import jpeg4py_loader_w_failsafe
from ...train.admin import env_settings # SHITCODE
from ..dataset_deprecated.depth_utils import get_x_frame

class DepthTrack_origin(BaseImageSequenceDataset):
    """ DepthTrack dataset.
    """
    ...
    def __init__(self, root=None, dtype='rgbcolormap', split='train', image_loader=jpeg4py_loader_w_failsafe): #  vid_ids=None, split=None, data_fraction=None
        ...
        args:
            ...
            root     - path to the depthtrack dataset. (this script is copied from lasot dataset and modified)
        ...
        if split not in ['train', 'val', 'test']:
            # Need to mention that 'train' split is actually a part of Depthtrack official trainset. like:
    ...
    def _build_sequence_list(self):
        file_path = os.path.join(os.path.dirname(os.path.realpath(__file__)), 'assets', f'depthtrack_{self.split}.txt')
    ...
    def _read_bb_anno(self, seq_path):
        ...
        gt = pd.read_csv(bb_anno_file, delimiter=',', header=None, dtype=np.float32, na_filter=True, low_memory=False).values
