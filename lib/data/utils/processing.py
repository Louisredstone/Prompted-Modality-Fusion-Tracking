...
from . import processing_utils as prutils
...
class ViPTProcessing(BaseProcessing):
    ...
    def _get_jittered_box(self, box, mode):
        ...
        """
        if not isinstance(box, Tensor): box = Tensor(box)
    ...
    def __call__(self, data: TensorDict):
        """
        args:
            data - The input data, like:
                TensorDict({'template_images': list of np.ndarray [(H,W,6)],
                            'template_anno': list of torch.Tensor [(4,)], 
                            'search_images': list of np.ndarray [(H,W,6)],
                            'search_anno': list of torch.Tensor [(4,)],
                            'dataset': str,
                            'test_class': meta_obj_test.get('object_class_name')})
        returns:
            data (modified, with the same keys.)
        """
