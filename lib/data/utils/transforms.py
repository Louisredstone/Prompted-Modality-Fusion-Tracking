...
class TransformBase:
    ...
    def __call__(self, **inputs):
        ...
        for var_name, var in input_vars.items():
            if var is not None:
                transform_func = getattr(self, 'transform_' + var_name) # transform_image, transform_bbox, etc.
...
class ToTensorAndJitter(TransformBase):
    ...
    def transform_image(self, image, brightness_factor):
        # handle numpy array
        image = torch.from_numpy(image.transpose((2, 0, 1))) # guess: GBR -> RGB
...
class Normalize(TransformBase):
    ...
    def transform_image(self, image):
        ...
        if dims[0] == 6: # TODO: code optimization: treat multi-modal data separately
            rgb = image[:3, :, :]
            depth = image[3:, :, :]
...
class ToGrayscale(TransformBase):
    ...
    def transform_image(self, image, do_grayscale):
        ...
        if do_grayscale:
            dims = list(image.size())
            visible = image[:3, :, :]
            visible_gray = cv.cvtColor(visible, cv.COLOR_RGB2GRAY)
            if dims[0] == 6:
                auxiliary = image[3:, :, :]
                auxiliary_gray = cv.cvtColor(auxiliary, cv.COLOR_RGB2GRAY)

                return np.stack([visible_gray, visible_gray, visible_gray, auxiliary_gray, auxiliary_gray, auxiliary_gray], axis=2)
            else:
                return np.stack([visible_gray, visible_gray, visible_gray], axis=2)
        else:
            return image
...
class ToBGR(TransformBase):
    ...
    def transform_image(self, image):
        if torch.is_tensor(image):
            raise NotImplementedError('Implement torch variant.')
        assert image.shape[2] % 3==0, 'Input image must have 3n channels.'
        # img_bgr = cv.cvtColor(image, cv.COLOR_RGB2BGR)
        # return img_bgr
        return np.stack([cv.cvtColor(image[:, :, i:i+3], cv.COLOR_RGB2BGR)
                         for i in range(0, image.shape[2], 3)], axis=2)
