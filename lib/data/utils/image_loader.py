...
def smart_loader(path):
    """ Smart image loader that tries to use jpeg4py first, and falls back to opencv if necessary."""
    if '.' not in path: raise ValueError('Invalid path: {}'.format(path))
    ...
    if ext in ['bmp']:
        im = Image.open(path)
        return np.array(im)
    # elif ext in ['png', 'gif', 'tif', 'tiff']:
    elif ext in ['png', 'gif', 'tif', 'tiff', 'jpg', 'jpeg']:
        im = cv.imread(path, cv.IMREAD_COLOR)
        # convert to rgb and return
        return cv.cvtColor(im, cv.COLOR_BGR2RGB)
    else:
