...
def sample_target(im: ndarray[('C', 'H', 'W'),], 
                  target_bb, 
                  search_area_factor, 
                  output_sz=None, 
                  mask=None):
    ...
    x1_pad = max(0, -x1)
    x2_pad = max(x2 - hwc_im.shape[1] + 1, 0)

    y1_pad = max(0, -y1)
    y2_pad = max(y2 - hwc_im.shape[0] + 1, 0)

    # Crop target
    im_crop = hwc_im[y1 + y1_pad:y2 - y2_pad, x1 + x1_pad:x2 - x2_pad, :]
    if mask is not None:
...
def transform_image_to_crop(box_in: torch.Tensor, box_extract: torch.Tensor, resize_factor: float,
    ...
    """
    if not isinstance(box_in, Tensor): box_in = Tensor(box_in)
    if not isinstance(box_extract, Tensor): box_extract = Tensor(box_extract)
    if not isinstance(crop_sz, Tensor): crop_sz = Tensor(crop_sz)
