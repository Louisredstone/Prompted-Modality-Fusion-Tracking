...
from ...data.utils.processing import BaseProcessing
from ..data_resource import BaseImageSequenceDataResource
...
class TrackingSampler(torch.utils.data.Dataset):
    ...
    def __init__(self, 
                #  data_resources: list[BaseImageSequenceDataResource], 
                 data_resources, 
                 ratios, samples_per_epoch, max_gap,
                 num_search_frames, num_template_frames=1, processing=no_processing, frame_sample_mode='causal',
                 train_cls: bool=False, pos_prob=0.5):
        """
        args:
        ...
        self.ratios = [x / ratio_sum for x in ratios]

        self.samples_per_epoch = samples_per_epoch
        self.max_gap = max_gap
        self.num_search_frames = num_search_frames
        self.num_template_frames = num_template_frames
        self.processing: BaseProcessing = processing
    ...
    def getitem(self):
        ...
        while not valid:
            ...
            data_resource = random.choices(self.data_resources, self.ratios)[0]

            # is_image_sequence_dataset = data_resource.is_image_sequence_dataset()
            is_image_sequence_data_resource = isinstance(data_resource, BaseImageSequenceDataResource)

            # sample a sequence from the given data resource
            seq_id, visible, seq_info_dict = self.sample_seq_from(data_resource)


            if is_image_sequence_data_resource:
            ...
            # try:
            template_frames, template_anno, meta_obj_train = data_resource.get_frames(seq_id, template_frame_ids, seq_info_dict)
            search_frames, search_anno, meta_obj_test = data_resource.get_frames(seq_id, search_frame_ids, seq_info_dict)

            data = TensorDict({'template_images': template_frames,
                                'template_anno': template_anno['bbox'],
                                'search_images': search_frames,
                                'search_anno': search_anno['bbox'],
                                'dataset': data_resource.get_name(),
            ...
            #     valid = False # CRITICAL WARNING: Need to log this information into log file!
            #     raise e # DEBUG
        return data
    ...
    def getitem_cls(self):
        ...
        returns:
            TensorDict - dict containing all the data blocks
        """
        raise NotImplementedError("Classification not implemented for TrackingSampler. 'Dataset' needs to be modified to 'Data Resource' type in this function.")
        valid = False
        label = None
        while not valid:
            # Select a dataset
            dataset = random.choices(self.data_resources, self.ratios)[0]

            is_video_dataset = dataset.is_video_sequence()

            # sample a sequence from the given dataset
            seq_id, visible, seq_info_dict = self.sample_seq_from(dataset, is_video_dataset)
    ...
    def sample_seq_from(self, data_resource: BaseImageSequenceDataResource):
    ...
    def get_one_search(self):
        raise NotImplementedError("'Dataset' needs to be modified to 'Data Resource' type in this function.")
        # Select a dataset
        dataset = random.choices(self.data_resources, self.ratios)[0]
