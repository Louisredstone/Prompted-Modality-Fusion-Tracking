...
def default_config():
    ...
    """
    logger.warning("vipt.default_config is deprecated but called.")
    cfg = edict()
...
#     cfg_dict = {}
#     _edict2dict(cfg_dict, default_config())
    ...
    if isinstance(base_cfg, dict) and isinstance(exp_cfg, edict):
        for k, v in exp_cfg.items():
            if k in base_cfg:
                ...
                raise ValueError("{} not exist in default config".format(k))
    else:
        raise TypeError("base_cfg and exp_cfg should be dict and edict")


# def update_config_from_file(filename, base_cfg=None):
#     if base_cfg is None: base_cfg = default_config()
