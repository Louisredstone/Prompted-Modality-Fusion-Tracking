...
def combine_tokens(template_tokens, search_tokens, mode='direct', return_res=False):
    ...
    if mode == 'direct':
        combined_tokens = torch.cat((template_tokens, search_tokens), dim=1)
        ...
        combined_tokens = torch.cat((first_half, template_tokens, second_half), dim=1)
    elif mode == 'partition':
        ...
        if return_res:
            return combined_tokens, merged_h, merged_w
