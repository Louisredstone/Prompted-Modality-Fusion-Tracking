...
def get_data(url):
    ...
    for sequence in meta["sequences"]:
        ...
        # get pic zip file
        url = sequence["channels"]["depth"]["url"]
        if bool(urlparse(url).netloc):
            frame_url = url
        else:
...
def write2csv(csvfile, fnames, urls):
    ...
    #print(len(fnames), len(urls), len([False] * 50))
    df = pd.DataFrame({"filename": fnames, "urls": urls, "state": [False] * 127})
...
def download_annos(use_proxy=True):
    ...
    for fname, (url, state) in annos.iterrows():
        ...
        print('download-annos path: ',os.getcwd(),root, fname,folder_path)
...
def download_sequences(use_proxy=True):
    ...
    for fname, (url, state) in sequences.iterrows():
        ...
        if state:
            ...
            ## download
            download(url, folder_path, "depth", use_proxy)
            ## unzip
            zip_src = folder_path + "/depth.zip"
            dest_dir = os.path.join(os.getcwd(), root, fname, "depth")
            state = unzip_file(zip_src, dest_dir)
...
if __name__ == "__main__":
    ...
    # stack = "vot2022/lt"
    # version = "VOT2022_LT"
    stack = input("请输入要下载的数据集名称（e.g.vot2022/lt）：")  # 注意：这里的名称要和 `VOT_DATASETS` 里面的 `key` 对应
    version = input("请输入要保存的数据集名称（e.g.VOT2022_LT）：")  # 这里随便起一个名字，易懂即可，比如 `votlt2022`

    #base_root = input("请输入数据集保存的路径（e.g./data）：")
    root = f"./{version}"
    sequence = ["channels.color=color/%08d.jpg\r\n", "channels.depth=depth/%08d.png\r\n","format=default\r\n", "fps=30\r\n"]
    csvfile = {
    ...
    ## 3.下载数据
    #download_annos(use_proxy=False)  # 3.1 下载 groundtruth.txt
    #print("Done, groundtruth.txt has been downloaded!")

    download_sequences(use_proxy=False)  # 3.2 下载 sequences
    print("Done, depth.zip has been downloaded!")
