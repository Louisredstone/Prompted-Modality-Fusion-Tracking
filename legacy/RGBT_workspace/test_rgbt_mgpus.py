...
def run_sequence(seq_name, seq_home, dataset_name, yaml_name, num_gpu=1, epoch=300, debug=0, script_name='prompt'):
    ...
    if len(RGB_img_list) == len(RGB_gt):
        ...
        res = np.zeros((len(RGB_img_list), 8), dtype=RGB_gt.dtype)
    else:
    ...
    res[0][0] = result[0][0]
    res[0][1] = result[0][1]
    res[0][2] = result[0][0]+result[0][2]
    res[0][3] = result[0][1]
    res[0][4] = result[0][0]+result[0][2]
    res[0][5] = result[0][1]+result[0][3]
    res[0][6] = result[0][0]
    res[0][7] = result[0][1]+result[0][3]
    toc = 0
    for frame_idx, (rgb_path, T_path) in enumerate(zip(RGB_img_list, T_img_list)):
        ...
        if frame_idx == 0:
            ...
            if dataset_name == 'RGBT234' or dataset_name == 'GTOT':
                res[frame_idx][0] = result[frame_idx][0]
                res[frame_idx][1] = result[frame_idx][1]
                res[frame_idx][2] = result[frame_idx][0]+result[frame_idx][2]
                res[frame_idx][3] = result[frame_idx][1]
                res[frame_idx][4] = result[frame_idx][0]+result[frame_idx][2]
                res[frame_idx][5] = result[frame_idx][1]+result[frame_idx][3]                                    
                res[frame_idx][6] = result[frame_idx][0]
                res[frame_idx][7] = result[frame_idx][1]+result[frame_idx][3]
...
if __name__ == '__main__':
    ...
    if dataset_name == 'GTOT':
        seq_home = '/home/public/dataset/RGBT_Datasets/GTOT' # MEMO 什么是sequence?
        seq_list = [f for f in os.listdir(seq_home) if isdir(join(seq_home,f))] # TODO 复用该生成函数
        seq_list.sort()
    elif dataset_name == 'RGBT234':
        seq_home = '/home/public/dataset/RGBT_Datasets/RGBT234'
        seq_list = [f for f in os.listdir(seq_home) if isdir(join(seq_home,f))]
        seq_list.sort()
    elif dataset_name == 'LasHeR':
        seq_home = '/home/public/dataset/RGBT_Datasets/LasHeR/TestingSet/testingset'
        seq_list = [f for f in os.listdir(seq_home) if isdir(join(seq_home,f))]
        seq_list.sort()
    elif dataset_name == 'VTUAVST':
        seq_home = '/mnt/6196b16a-836e-45a4-b6f2-641dca0991d0/VTUAV/test/short-term' # TODO 绝对路径改为datasets内相对路径.
        with open(join(join(seq_home, 'VTUAV-ST.txt')), 'r') as f: # MEMO why two joins?
            seq_list = f.read().splitlines()
    elif dataset_name == 'VTUAVLT':
        seq_home = '/mnt/6196b16a-836e-45a4-b6f2-641dca0991d0/VTUAV/test/long-term' # TODO 绝对路径改为datasets内相对路径.
